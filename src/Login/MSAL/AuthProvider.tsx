    ...
    return class AuthProvider extends Component<IAuthProvider, {}> {
              ...
              if (isIE() && getAuthContext().idToken.rawIdToken != "") {
                ...
                if (msalApp.getAccount()) {
                  ...
                  if (account.hasOwnProperty("idToken")) {
                    const msalidtoken =
                      "msal." + account.idToken.aud + ".idtoken";
                    console.log(
                      "Getting cookie",
                      msalidtoken,
                      " Token Key" + this.getCookieValue(msalidtoken)
                    );
                    const token = {
                      idToken: {
      ...
      async onSignIn(redirect: any) {
        ...
        if (msalApp.getAccount() && !msalApp.isCallback(window.location.hash)) {
            console.log('inside msalApp.getAccount() ==> true');
          console.log("Inside acquireToken");
        //   alert('inside msalApp.getAccount() ==> true');
          const tokenResponse = await this.acquireToken(
            GRAPH_REQUESTS.LOGIN,
            true
          ).catch((error) => {
            console.log("Got error in signin", error);
            this.setState({
              error: error.message,
            });
          });
          if (tokenResponse) {
            ...
            // alert('inside msalApp.getAccount() ==> false');
            console.log('inside msalApp.getAccount() ==> false');
            console.log('GRAPH_REQUESTS.LOGIN==>', GRAPH_REQUESTS.LOGIN);
